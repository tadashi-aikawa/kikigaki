...
| AIに質問 | Control + Option + Command + A。AIを有効にした会議のみ |
...
herdrとCodexまたはClaude Codeを導入し、使うCLIで [kikigaki Skill](skills/kikigaki/SKILL.md) を利用できるようにします。設定へ次を追加すると、次の会議から「AIに質問…」が使えます。`[ai]` を省略した場合は無効です。
...
録音停止後も回答を回収します。次の会議への回答と混ぜず、旧会議の回答は元のMarkdownへ保存します。保存に失敗したときは「保存を再試行」を使えます。アプリ再起動後は未完了会議の返送を回収しますが、AIセッションの再接続や質問の再送はしません。
