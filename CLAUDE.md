...
`herdrCommand` はプロファイル共通です。adapterを全チャネルで共有するため、2つ目以降に別の値を書くと設定エラーになります。ホットキーも1つ目のプロファイルのものだけを使い、宛先を選び直しても変わりません。

会議に紐づかないAIセッションを先に起こしておく「準備済みセッション」は設計だけ済んでいて、**画面と起動の実装はまだありません**。設計は [AI設定の複数プロファイル](docs/ai-profiles.md) を参照してください。稼働中のherdrペインへ接続する `attach` と `displayAgent` は取り下げたため、書くと設定エラーになります。
