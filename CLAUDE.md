...
返事待ちは進行文と4分割バーで確認できた位置を示します。段は「送信 · 読込 · 編集 · 返答」で、バーの下に9ptの段名を表示します。読込はAIの `accept`、編集はAIの1回の自己申告かClaudeの編集系ツールのフックで確認します。総数の申告があれば「編集中(全7か所)」と出します。返答が届いたら全段を1.5秒点灯してから本文へ入れ替えます。経過時間は表示中だけ更新し、過去会議は静止します。詳細は [AI依頼の進行表示](docs/ai-progress.md) を参照してください。
