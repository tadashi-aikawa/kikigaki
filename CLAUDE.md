...
`autoStart = true` のプロファイルがあれば録音開始と同時に自動送信を始めます。それ以外はフッター左端のロボットをクリックし「自動実行…」から開始します。「手動実行…」は従来のAI依頼シートです。返事待ちは朱のロボットと「実行中」、返事待ちがなく自動送信が稼働中なら下に `2:30`・`0:45` の形式で残り時間を表示します。非稼働なら下ラベルはありません。円形ゲージと別のAI送信アイコンは廃止しました。「今すぐ送る」で即時送信し、次の期限を操作時点から1間隔に打ち直します。ダブルクリックでは送信しません。返事待ち・差分なしの回はスキップします。
...
ロボットの目は表示中の返事待ちに限り1秒周期で左右へ動きます。非表示・最小化・非稼働ではタイマーを止め、「視差効果を減らす」では目を静止させます。連続アニメーションは使いません。ホットキー ctrl+alt+cmd+A は引き続き手動実行シートを開きます。
