...
@MainActor final class AIPastMeetingsWindow: NSWindowController {
    ...
    @objc private func selected() {
        ...
        // 過去会議でも編集・返答の申告は受信箱に残っているので、保存済みイベントから静止で再現する。
        ...
        let audioLevels = meeting?.displaysAudioLevels == true
            ? meeting?.audioLevels?.assessments(for: utterances, exclusion: meeting?.audioExclusion) ?? [] : []
            ...
            if let reply = row as? AIReplyRow {
                ...
                reply.updateProgress(question.map {
                    AIProgress(question: $0, connection: .unknown, connectionGeneration: nil,
                               report: state.progressReports[item.requestID], isHistorical: true)
                }, reduceMotion: true)
        ...
        for (index, utterance) in utterances.enumerated() {
            ...
            row.updateExclusion(meeting?.audioExclusion?.excludes(utterance, track: meeting?.audioLevels) ?? false)
            rows.append(row)
