...
final class AIPreparedStore {
    ...
    func refresh() async {
        ...
        guard let panes = try? await herdr.panes() else { return }
    ...
    func prepare(profile: ResolvedAIConfig, helper: URL, outputDirectory: URL, now: Date = Date()) async {
        guard isUsable, !launching.contains(profile.slot) else { return }
    ...
    func bind(_ id: UUID, to meetingID: UUID, config: ResolvedAIConfig) throws {
        try ledger.bind(id, to: meetingID, config: config)
        try save()
        onChange?()
    }

    func discard(_ id: UUID) {
        do { try ledger.discard(id); try save(); warning = nil }
