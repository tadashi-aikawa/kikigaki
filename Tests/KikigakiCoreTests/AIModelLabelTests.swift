...
@Suite struct AIModelLabelTests {
    ...
    /// 相手の塊は録音開始シートの宛先と同じ表記。effortは括弧でモデルに掛ける。
    @Test func CLI名とmodelを中黒で結びeffortは括弧で添える() {
        let value = label(AIConfig(cli: .codex, model: "gpt-6-astra", effort: "high", cwd: "~/work/minutes"))
        #expect(value.provider == "Codex" && value.model == "gpt-6-astra" && value.effort == "high" && value.directory == "minutes")
        #expect(value.text == "Codex · gpt-6-astra (high) · minutes")
    }

    @Test(arguments: [AIProvider.codex, .claude]) func model未設定ならCLI名だけを出す(cli: AIProvider) {
        let value = label(AIConfig(cli: cli, effort: "high", cwd: "~/work/minutes"))
        #expect(value.provider == cli.rawValue.capitalized && value.model.isEmpty)
        #expect(value.text == cli.rawValue.capitalized + " (high) · minutes")
    }
    ...
    @Test func effort未設定なら飛ばし末端ディレクトリは既定cwdでも出す() {
        ...
        #expect(value.text == "Codex · gpt-6-astra · ai-work")
        #expect(value.stages == [stage("Codex · gpt-6-astra", "ai-work"), stage("Codex · gpt-6-astra")])
    }

    @Test func ルート直下のcwdは末端ディレクトリとして出さない() {
        let value = label(AIConfig(cli: .claude, model: "sonnet-5", cwd: "/"))
        #expect(value.directory == nil && value.stages == [stage("Claude · sonnet-5")])
    }
