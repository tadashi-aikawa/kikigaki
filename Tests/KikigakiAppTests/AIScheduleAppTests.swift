...
@Suite(.timeLimit(.minutes(1))) @MainActor struct AIScheduleAppTests {
    ...
    @Test func 自動と手動は同じ接続を使い変更なしとシート中は送らない() async throws {
        ...
        let answered = try #require(session.aiRecord?.controller.conversation.questions.first)
        #expect(AIInlineMark(question: answered, kind: .result).title.hasSuffix(" · 自動"))
        #expect(AIInlineMark(question: answered, kind: .result).excerpt == "議事録を更新しました")
        #expect(AIInlineMark(question: answered, kind: .question).excerpt.hasPrefix("対象: "))
