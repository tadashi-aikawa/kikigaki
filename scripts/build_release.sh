...
VERSION="${1:-}"
APP="$ROOT_DIR/.build/KIKIGAKI.app"
DIST_DIR="$ROOT_DIR/dist"
ARCHIVE="$DIST_DIR/KIKIGAKI-$VERSION.zip"
...
# CI 以外(ローカル検証)では kikigaki-dev 証明書が無いことがあるためスキップする
if [[ "${CI:-}" == "true" ]]; then
...
# --keepParent が無いと展開時に KIKIGAKI.app の中身がばらける
