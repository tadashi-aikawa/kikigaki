...
| 有 | 未紐づけ | 候補に出る | 同じものを選び直せば冪等に通す。「新規に起動する」を選んだら**会議側の接続を手放す** |
| 有 | 紐づけ済み | 閉じた表題に出る | 完了。候補から消える |
| 無 | 紐づけ済み | 候補に出ない | 起きない。台帳は会議側の保存が終わってから書く |

「新規に起動する」を選んだ枠で会議側の接続を残すと、新規を選んだのに準備済みのペインへ送ってしまう。手放すのは、その世代でまだ依頼を1つも作っていない枠だけにする。

録音を取り止めたときは、会議の実体・AI会議の登録簿・監視・紐づけの4つを揃えて戻す。実体だけ消すと監視が残り、再起動時に無いmanifestを回収しようとして失敗する。紐づけ済みの準備済みも、会議が無くなった以上は未紐づけへ戻して次の録音で選べるようにする。片付け先は**会議に固定したMarkdownの親**から組み立てる。録音中に保存先を再読込しても、この会議が書いた場所は動かない。
