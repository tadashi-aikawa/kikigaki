...
ポップアップに並ぶのは設定のプロファイルだけである。そのプロファイルに未紐づけの準備済みセッションがあれば、行に「議事録 (準備済み 13:05)」と添えて、どれを使うか選べるようにする。選んだ時点で紐づける。
...
一覧には未紐づけの準備済みだけを出し、プロファイル名・起動時刻・接続状態を並べる。行ごとに「破棄」と「ペインを開く」を置く。破棄は台帳から外すだけで、ペインのプロセスは終了させない。外部で起きていることの後始末はherdr側で行う。
...
送信シートの宛先ポップアップでも、そのプロファイルに未紐づけの準備済みがあれば「議事録 (準備済み 13:05)」として選べる。**選んだ時点で紐づける**。紐づけた後は、そのプロファイルのチャネルとして既存の契約がそのまま適用される。世代は1から始まり、`AIStreamHistory` も受領基準も新しい会議のものになる。
...
| 7・アプリ | 準備の起動経路(会議に紐づかない `connect`)、台帳の保存と起動時の生存確認、「AIセッションを準備…」と一覧、録音開始の紐づけシート、宛先ポップアップの準備済み表示、`autoStart` の順序、送信との排他 |
