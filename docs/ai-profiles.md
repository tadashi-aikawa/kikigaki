...
| 6・Core | 台帳 `AIPreparedSessions` の値型(復号・検証・未紐づけの抽出・紐づけ・破棄)、`attach` / `displayAgent` の拒否、`AIAgentResolver` と `AIHerdr.list` の宛先候補の削除 |
...
段6の実装へ入る前にタダシへ確認する。画面の見え方はクロディーヌのモックを併せて見てもらう。

1. **同じプロファイルの準備済みが複数あるときの、録音開始シートの既定**。推奨は**最も古い1件**を初期選択にする。理由: 連続する会議のために先に用意したものから順に使うのが自然で、余った新しいほうは次の会議へ回せる。代替は最も新しい1件(直前に用意したものを使う想定)と、既定を選ばず利用者に選ばせる案。後者は `autoStart` が選択待ちで止まるので推さない。
2. **「AIセッションを準備…」を待機中のどこに置くか**。推奨はメニューバーのメニューに置き、書き起こしウィンドウのフッターにも録音中だけ出す。理由: 待機中は書き起こしウィンドウを開いていないことが多く、フッターだけだと辿り着けない。代替はウィンドウ上部の常設ボタンだが、録音の主操作の近くに会議外の操作を混ぜたくない。
3. **紐づけシートで「新規に起動する」を選んだとき、未紐づけの準備済みをどう扱うか**。推奨は**そのまま残す**。理由: 次の会議で使えるし、破棄は明示操作にしたほうが事故が少ない。代替はその場で破棄を促す確認を出す案で、溜まりすぎを防げるが操作が増える。
4. **準備済みセッションの一覧をどこで見せるか**。推奨は準備シートと同じ画面の下半分に置き、そこから破棄と「ペインを開く」を行う。理由: 起こす操作と溜まっているものの確認は同じ関心で、画面を分けるとどこにあるか分からなくなる。代替は「前の会議に返事あり」と同じ列にピルを置く案。
5. **紐づけシートを出している間に録音を停止した場合の扱い**。推奨はシートを閉じ、紐づけずに終える。準備済みはそのまま残す。理由: 会議が無いのに紐づける先が無い。代替は選択を待ってから停止する案だが、停止操作が利用者の入力待ちで止まるのは避けたい。
