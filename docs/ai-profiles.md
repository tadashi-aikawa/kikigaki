...
一覧には未紐づけの準備済みだけを出し、プロファイル名・ペインの表題・起動時刻・接続状態を並べる。行ごとに「破棄」と「ペインを開く」を置く。破棄やペイン消失で未紐づけを台帳から外したら、その準備用の置き場も削除する。紐づけ済みの置き場は残し、ペインのプロセスは終了させない。外部で起きていることの後始末はherdr側で行う。
