...
本文とフッターの間に、placeholderが「会話に書き込む… · ⌘Enterで投稿」の1行入力欄を置く。tooltipとアクセシビリティのヒントも「⌘Enterで投稿」とする。送信先を選ぶ操作や設定は追加しない。投稿してもAIへの送信は始めず、会話への追加だけを行う。
...
| Enter・Shift+Enter | 投稿も改行もしない。IME変換中は確定だけを行う |
...
手入力の行は、墨の丸地に紙色の鉛筆 `pencil`、固定名「手入力」、時刻、本文で構成する。吹き出しの尾と発話の確定段ゲージは付けず、名前・アバターからの改名操作を出さない。話者数にも加えない。
...
| UI | IME確定Enterで未投稿、⌘Enterで1回投稿、素のEnter・Shift+Enterは投稿も改行もしない、Escで下書き保持、複数行貼り付け、長いURL、投稿拒否時の下書き保持 |
