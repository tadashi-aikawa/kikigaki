...
起動直後と会議の準備中でstoreが無い間の指定は、メモリに保留して開始する会議へ引き継ぐ。適用済みの保留は消す。

[録音開始シート](start-sheet.md)の指定も同じ待機指定の経路を使うが、預ける先だけ別にする(`prepareMinutes`)。停止後も前の会議のMarkdownは残っているため、シートの指定をその場で待機指定にすると、次の表示更新で前の会議の議事録になってしまう。`start` が会議を切り替える時点で待機指定へ移し、そこから先は従来どおり `previewMinutesStore` が引き継ぐ。指定があれば開始と同時に右ペインを開く。取消は録音を始めないので、引き継ぎも起きない。

`kikigaki://start?minutes=<パーセントエンコードした絶対パス>` のリンクも、開始シートのパス欄へ入れるところまでで止める。ここから直接 `selectMinutes` を呼ばないのは、表示中の会議の対象を外から書き換えないため。検証は `MinutesPath.validate` に合わせ、通らない指定はパス欄を触らず理由だけを出す。待機中以外に届いたリンクは受けない。詳細は [録音開始シート](start-sheet.md) を参照。
...
左ペインのヘッダー右端へ「議事録」ボタンを1つ追加する。SF Symbolは `sidebar.right` 系を用い、「議事録を表示」または「議事録を隠す」のツールチップとアクセシビリティラベルを付ける。メニューバーアイコンのStatusItemメニューにも「議事録を表示／隠す」を置き、閉じたウィンドウを開いてから切り替える。状態と表題はヘッダーと共有する。フッターの「…」には追加しない。無効時は色を抜き、無効状態だけの背景面を足さない。

左右の `NSSplitView` にし、左には従来のヘッダー・検索・会話・入力・フッターをまとめて置く。右は上部の編集可能パス欄と「ファイルを選ぶ…」と×ボタン、下部の独立したスクロール領域。×はペインを隠さず、表示中の議事録を閉じて対象を解除する(空欄の確定と同じ経路。起動直後と同じ空の状態へ戻る)。表示対象が無いときは押せない。ペインの表示切替はヘッダーの「議事録」ボタンとメニューだけが行う。NSOpenPanelは単一の `.md` ファイルに限定し、取消では対象を変えない。未作成パスはパス欄から指定する。編集中の通知で入力文字を置き換えず、パス欄の下に対象が変わった旨を1行表示し、編集の確定・取消時に反映する。
...
| 人の指定ファイルなし | 「指定したファイルはまだありません。作成されると自動で表示します」。パスを残して監視を続ける。 |
| AIの通知ファイルなし | 「AIが通知したファイルはまだありません。作成されると自動で表示します」。パスを残して監視を続ける。 |
| 読取失敗 | 権限・文字コード・サイズ等の原因を簡潔に表示し、再読込できる。 |

非表示中のAI通知はフッターのスペーサー右、ピンの手前へ、薄墨の `doc.text` と下ラベル「議事録」を表示する。クリックでプレビューを開き、同じ印を消す。通知音と前面化はしない。表示中の通知は対象を切り替えて描画し、印は立てない。人のファイル選択だけでは印を立てない。印は内容を確認した証明やファイル実在の保証ではない。

右上部バーは題を置かず1行にし、左ヘッダーと区切り線の高さを揃える。パス欄は和紙地と細い輪郭、フォーカス中だけ墨の輪郭にする。上部バーと本文は左余白24ptで揃える。本文と表の最大幅は720ptとし、左寄せする。コードも現在は折り返し、本文全体の横スクロールは設けない。対象なしの案内は上から約1/3へ置き、直下にもファイル選択を置く。

### 最近表示したファイルの履歴

パス欄へフォーカスすると、最近プレビューに表示できたファイルを最大10件、欄の直下に和紙色の一覧で出す。人のパス入力・ファイル選択と、AIの `minutes` 通知による切替を同じ扱いにする。選択や読込開始だけでは記録しない。`MinutesWebView.onRendered` の現行描画完了時に記録し、空ファイルの正常表示も含める。読込・描画失敗、非表示中の通知、対象変更や停止で無効になった描画は含めない。

保存はアプリ全体のUserDefaultsの `KikigakiMinutesHistory` 1キーに絶対パスの配列を置く。新しい順で、同じパスの再表示は先頭へ移し、10件を超えた分を末尾から落とす。Coreの純粋な値型 `MinutesHistory` が順序・重複・上限を扱い、アプリ側が読書きする。読み出すたびに最新の配列を使い、複数の会議ビューが古い写しで上書きしない。復元時の不正パスは既存の `MinutesPath` 検証で除く。symlink解決や実在確認で同一性を変更しない。

各行はファイル名を12ptの墨、親ディレクトリを11ptの薄墨で表示する。親が直前の行と同じなら従段を省き、44ptの行を28ptへ縮める。長い親パスは先頭を省略し、全パスはツールチップへ置く。ホバーとキーボード選択は薄い地で示し、一覧には影を付ける。存在しないファイルも削除せず、ファイル名は墨のまま行末に11ptの薄墨で「見つかりません」を添える。選択後は既存の未作成案内に任せる。

開いた直後は先頭を選択し、Returnだけで最近のファイルを開ける。クリック、または上下キーで選んでReturnを押すと、パス欄に値を入れて既存の確定経路で開く。選択変更時はVoiceOverへファイル名と不存在の注記を通知する。入力を編集すると選択を解除し、Returnは入力中のパスを確定する。Escapeは一覧を閉じる操作を先に行い、もう一度押すと既存の下書き取消になる。一覧の余白クリックでも閉じ、欄のフォーカスは残す。IME変換中は候補操作を優先する。フォーカスが欄から外れたとき、ウィンドウが非キーになったとき、会議切替、プレビュー非表示でも閉じる。履歴が空、または表示可能な高さが44pt未満なら一覧を出さず、上下キーとReturnを奪わない。同じパスが履歴の先頭なら再描画時にUserDefaultsへ書き直さない。ヘッダーのボタン配置は変更しない。

会議の `minutes.json`、AI通知の契約、envelopeの `minutes_path` は変更しない。履歴は表示の便宜であり、会議の記録やAIの書き先を自動的に引き継ぐ仕組みではない。個別削除はこの段には含めない。検証ハーネスとテストには隔離したUserDefaultsを注入する。
...
- フルスクリーン・タイル中はウィンドウ幅を変えず、分割だけを切り替える。希望幅は保存しない。解除時とライブリサイズ終了時に希望幅を画面へ合わせ直す。タイルは公開状態がないため、画面全高かつ半幅または全幅で左右端に接する配置から推定する。同じ配置を手動で作った場合との区別はできない。
...
- 通知から約200msのデバウンスを置く。読取はUIスレッドの外で行い、Markdown解析と図・数式の描画は議事録専用WebKitで行う。会議ID・パス・読込世代が今の対象と一致する結果だけ反映する。
- 監視のopen前にstatで通常ファイルかディレクトリかを確認し、差し替えにも備えてO_NONBLOCKを付ける。読取は通常ファイルを確認し、FIFO等を待たない。ユーザーの議事録は0600や単一hardlinkを要求しない。symlinkは読み取り用途として解決先を辿り、元の指定パスを保持して付け替えを再検知する。
...
- 読取失敗はstampが変わるまで再読込しない。ファイル無しとiCloud待ちだけを定期再試行する。
- 読取中の置換はfdとパスの再照合で検出し、旧inodeの結果を反映せず再読込する。非原子的な書込の途中なら次の通知・定期走査で再読込する。200msは保存完了の保証ではない。
- 同じファイルの更新時は先頭可視ブロックとその画面内オフセットをアンカーにして戻し、短縮時だけ有効範囲へ収める。本文選択も範囲を補正して維持する。末尾への自動追従はしない。別ファイルへの切替時は先頭へ戻す。
- 非表示中は本文読取・解析・描画を停止し、表示ONで必ず最新ファイルを読む。受信箱の回収は継続する。ウィンドウ破棄・会議切替ではTimer、監視、保留読込を終了し、cancel完了後にfdを閉じる。

### Markdownの表示契約

議事録本文は専用WKWebViewで描画し、通常のAI返事の描画は変更しない。本文と表はペイン幅へ追従し、720ptの制限を設けない。ファイル先頭の閉じたfrontmatterは隠し、コード内の記法は変換しない。

脚注・callout・チェックボックス・見出しアンカー・画像・Mermaid・数式・SVGを表示する。任意HTMLは実行せず文字として残す。詳細な記法、画像の基準パス、同梱資産と実行境界は [議事録の描画と検索](minutes-rendering.md) を正本とする。

更新箇所の強調は、AIへの依頼の送信と、その依頼の編集の開始で基準を置き直し、以後は基準との差分を累積で残す。1つの依頼が議事録を何度書き換えても全箇所が見え、次の依頼の送信か編集の開始で消える。AIの依頼がない間は直前の描画との差分を4秒で消す従来のままにする。基準の置き直しは表示だけの扱いで、`ai/minutes.json` にも会議Markdownにも保存しない。

⌘Fは焦点のあるペインを検索する。右ペインにはNeovimとObsidianで対象ファイルを開くボタンを置く。
