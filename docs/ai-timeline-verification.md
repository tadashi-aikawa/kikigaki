...
- **確認質問への返答と再送の実herdr経路**。確認質問はモデルの判断で出るため誘発できず、再送も失敗を意図的に作る必要がある。テストで通してあるのは、行の「再送」がrequest IDをコールバックへ渡すところ(`AIViewTests`)と、宛先を固定したシートが選び直しと取消で動かないこと(`AIProfileReviewTests`)まで。`AppDelegate` のシート生成から実際の送信までは通していない
