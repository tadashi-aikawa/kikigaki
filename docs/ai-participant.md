...
未読回答の縦罫は朱、未解決の確認は金、既読や返答済みは薄墨にする。到着時の自動展開・前面化・スクロール移動はしない。印を開閉するときは、その見出しの位置を保つ。フッターの「AIへ渡す会話」と同じ行に件数のピルを置き、0件の種類は表示しない。未読は朱の塗り、確認待ちは金の塗りに白文字。回答待ちと送達不明は薄墨の枠、失敗は朱の枠にする。文字は11pt太字で、クリックすると該当する最初の印へ移動する。「前の会議に回答あり」も同じ列のピルで開く。確定待ちなどの進捗、接続の作り直し、保存の再試行もフッターで扱う。
