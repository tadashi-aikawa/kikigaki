...
フッターは左から、朱のロボット線画ボタン、余白付きの縦線、朱の丸の件数と下の「未読」、金のアイコンと件数・下の「要返答」、朱の警告アイコン、右端までの余白、「…」を1行に並べる。0件の状態は隠し、状態アイコンのクリックで該当する最初の行へ移動する。警告の全文はホバーで示す。ロボットのクリックで自動・手動の実行メニューをボタンの上へ開く。全スロットの返事待ちを「実行中」と目の動きで示し、それ以外で自動送信中は `分:秒` の残り時間、非稼働は下ラベルなし。無効な操作は面を足さず色を抜く。
