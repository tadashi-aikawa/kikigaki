...
未読回答の縦罫は朱、未解決の確認は金、既読や返答済みは薄墨にする。到着時の自動展開・前面化・スクロール移動はしない。印を開閉するときは、その見出しの位置を保つ。フッターの「AIへ渡す会話」と同じ行に未読・確認待ち・回答待ち・送達不明・失敗の件数を出し、0件の種類は表示しない。確定待ちなどの進捗、接続の作り直し、保存の再試行もフッターで扱う。
