...
送信時に会議・設定・音声位置・問いを固定する。`AIConfirmationWait` は最大3秒、確定状態を観察し、一時停止中も待ちの期限を進める。期限時の暫定部分は付帯情報とし、まだ処理していない音声を推測で補わない。取消と録音停止は準備タスクを取り消す。送信直前の生存確認を終えてから質問の状態を再確認し、送信試行を保存してpromptへ進む。
