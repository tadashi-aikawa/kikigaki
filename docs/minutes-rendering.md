...
| Mermaidコードブロック | 図として描画する。失敗時は記法を残す。timelineの期間と出来事は、描画前に実フォントで幅を測り150pxの枠に収まる位置へ `<br>` を差し込む。書かれた `<br>` は残し、`<br/>` も改行として扱う。title・section・コメント行は変えない。mindmapのノード幅上限は320px。 |
