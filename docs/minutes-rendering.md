...
右上の「目次」を押すとフローティング一覧を展開する。全階層の見出しを字下げして表示し、現在位置を強調する。見出し選択後も開いたままにし、目次ボタン・枠外クリック・目次内のEscapeで閉じる。見出しがない文書では表示しない。目次自体は本文検索の対象に含めない。本文の操作を重くしないよう目次は先頭300見出しまでとし、超えた場合は一覧内に案内する。
