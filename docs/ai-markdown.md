...
解析はフェンスを先に隔離し、`!!!` のadmonitionを見てから区切り行を確認して表を識別し、残りを行の役割へ分類する。箇条書きはインデントの増減から段数を保持し、番号は原文を維持する。引用は連続する引用行をまとめ、先頭が `[!種別]` のcalloutだけadmonitionにする。入れ子はadmonitionの本文だけに限り、任意のブロック入れ子を構築する汎用ASTには広げない。

admonitionは種別・原文の種別語・題・本文のブロック列を持つ。`!!! <種別> "<題>"` の題は引用符で囲んだものだけ受け、囲みの無い余りが続く行は段落へ戻す。本文は4桁の字下げで、間の空行を含み、字下げの無い行で終わる。題の直後の空行と末尾の空行は本文に含めない。calloutは折り畳みの `+`・`-` を捨て、残りの引用行を段数を1つ下げて組み直してから再帰する。題を省いたときは原文の種別語を題にし、`""` の指定では題を出さない。種別はnote / info / tip / hint / important / warning / attention / caution / danger / error / seealso / abstract / summary / example / question / quote / bug / success / failureの19語を大文字小文字を問わず受け、未知の種別はnoteの顔にして題に原文の種別語を出す。入れ子は6段までとし、それ以上は本文の段落として落とす。
...
hash・wikilink・Vault相対パス・画像・HTMLは専用解釈しない。HTMLの例外は `<br>` だけで、`<br/>`・`<br />` と大文字小文字を問わず行内の改行 `MarkdownInline.isLineBreak` にする。属性を書いた `<br class="x">` は受けず、行内コードとフェンスの中も文字のまま残す。画像記法を通常リンクとして部分的に拾わず原文で表示する。`[text](url)` は表示と行き先を分け、押せるのは妥当なhttp(s) URLだけとする。裸http(s) URLの末尾句読点と対応しない閉じ括弧はリンクへ含めない。
...
表の幅は内容で決める。表そのものには幅を指定せず、各列へ絶対値の `contentWidth` を置き、合計をTextKitに表の幅として使わせる。列の幅はその列のヘッダ・ボディの自然幅の最大で、見出しはsemiboldで測る。長いセル1つが他の列を押し潰さないよう1列260ptを上限にする。短い表は本文幅まで伸びず左に寄り、本文幅を変えても幅と高さが変わらない。

自然幅の合計が本文幅を超える表は、本文幅が決まってから列幅を置き直す。描画は幅を知らずに1回だけ行うため、`MarkdownBodyView` が描画結果から表の枡を列ごとに集めて自然幅を覚え、`height(for:)` で本文幅が決まった時点で配分する。配分は広い列から等しく詰める最大公平配分で、下限は24pt。狭い列は自然幅のまま残るので、比例配分のように短い見出しが1文字ずつ折り返すことがない。枡の幅は段落属性ではないので、置き直した幅は器の寸法だけでは組み直されない。変えたときだけ全体のレイアウトを無効化してから測る。

- 枡の余白5pt×2と罫0.5pt×2は、配分の予算から列数ぶん先に引く
- 下限を割ってもなお収まらない表は、TextKitが器の幅まで比例で詰めるので横へははみ出さない
- 絶対値の `contentWidth` を持つ枡だけを表として集める。コード・引用・水平線の器は割合の1セルなので混ざらない
- 幅0のまま測るとTextKitが器の寸法を誤る。行と同じく、測る前に表示する幅の枠を与える
