...
対象は `2540a86` 時点のコード。文字の確定と話者の固定は別の状態であり、「確定」は認識の正しさを保証しない。録音中に固定した話者も、停止時には全体の突き合わせで変わり得る。
...
以下は発話に含まれるトークン範囲に着目した図。文字と話者は並行して変化するため、録音中の枠を2領域に分けた。アプリに発話ごとの状態enumがあるわけではなく、確定数・凍結数・行添字から読み取った状態である。速報から高精度への置換で本文・トークン数・行境界も変わるため、同じ行が最後まで残るとは限らない。
...
根拠: [SpeakerFreeze.advance](../Sources/KikigakiCore/SpeakerFreeze.swift)。`elapsed` は消費済み音声のサンプル数から計算し、一時停止中の壁時計の経過は加算しない。凍結済みでも後続の未凍結トークンと同じ行にまとまれば、その行全体には「話者未確定」が付く。
...
| 通常行に未凍結トークンを含む | 通常の本文・推定話者に薄い背景と「話者未確定」 | `pendingSpeakerRows` に含まれる行。速報の確定でもこの表示になる |
| 通常行の全トークンが凍結済み | 「話者未確定」とその背景が消える | 「確定」の文字や認識精度は表示しない。停止時には再判定され得る |
| 話者判別オフの通常行 | マイク記号と「発言」、話者未確定の印なし | 話者が判明した意味ではなく、話者を判定しない会議 |
| オン時の話者ラベルがnil | 「?」 | 未凍結とは別の軸。「?」のまま凍結・最終表示になることもある |

### 内部では異なるが、行の見た目では区別しない状態

| 内部の区別 | 見えていないこと |
| --- | --- |
| 速報の確定と高精度の確定 | 出力元エンジンの印はない。オン時は未凍結なら両方とも「話者未確定」、オフ時は両方とも通常の「発言」 |
| モデル未判定・モデル暫定・モデル確定だが凍結猶予内 | 「話者未確定」は未凍結行を一括表示し、モデルの到達範囲や段階を出さない |
| 高精度待ち・猶予待ち・モデル確定範囲待ち・長い1文字の後続待ち | 凍結できない理由や残り秒数を表示しない |
| 一部だけ凍結した行と全体が未凍結の行 | どちらも「話者未確定」。行内の凍結境界は表示しない |
| 録音中の凍結と停止後の再判定済み | 行単位の専用印はない。会議全体の録音状態・保存結果は別に表示する |
| 正しく認識した文字・話者と誤認識 | 確定・凍結は正誤の採点ではなく、UIにも正解保証はない |

`TranscriptRow.update` は「話者未確定」の印だけが変わっても変更の点灯対象にしない。`TranscriptDocument` は渡された行を配置し、文字・話者の確定条件を判定しない。小音量による薄表示は別の除外状態なので、薄さだけから文字・話者の確定を読み取らない。詳細は [小音量発話の除外](audio-exclusion.md) を参照。

この区別を、別タスク「KIKIGAKI 発話が確定するまでの状態を表示する」の状態棚卸しの土台とする。この文書によるUI・コードの変更はない。
